...
def main():
    ...
    for _ in range(2):
        bench(
            False,
            f"{'torch':<12}",
            lambda: flashinfer.torch_norm.fused_add_rmsnorm(
                x, residual, weight, eps, True
            ),
        )
        bench(
            False,
            f"{'ffi':<12}",
            lambda: flashinfer.fused_add_rmsnorm(x, residual, weight, eps, True),
        )
        bench(
            False,
            f"{'ffi-wrapper':<12}",
            lambda: flashinfer.fused_add_rmsnorm(x_w, residual_w, weight_w, eps, True),
        )
        bench(
            False,
            f"{'ffi-stol':<12}",
            lambda: flashinfer.fused_add_rmsnorm(
                x_dl, residual_dl, weight_dl, eps, True
            ),
        )
        print()
